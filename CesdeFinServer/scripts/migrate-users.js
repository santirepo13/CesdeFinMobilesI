...
async function migrateUsers() {
    ...
    try {
        ...
        if (existingCount > 0) {
            console.log(`Collection already contains ${existingCount} users`);
            const answer = await question('Do you want to clear the collection before migrating? (y/N): ');
        ...
        for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
            ...
            for (const user of batch) {
                ...
                if (validationErrors.length > 0) {
                    ...
                    console.log(`Validation failed for user: ${user.usuario || 'UNKNOWN'}`);
                    continue;
                }
                ...
                duplicateCheckPromises.push(
                    usersCollection.findOne({ usuario: cleanedUser.usuario })
                        .then(existing => {
                            if (existing) {
                                migrationStats.duplicateUsers++;
                                console.log(`Duplicate user found: ${cleanedUser.usuario}`);
                                return null;
                            }
                            return cleanedUser;
                        })
            ...
            const checkedUsers = await Promise.all(duplicateCheckPromises);
            
            // Filter out null values (duplicates) and add to batch
            batchToInsert.push(...checkedUsers.filter(user => user !== null));
            
            if (batchToInsert.length > 0) {
                try {
                    const result = await usersCollection.insertMany(batchToInsert, { ordered: false });
                    ...
                    if (error.code === 11000) {
                        ...
                        console.log(` ${duplicates} duplicate users skipped`);
                        ...
                        if (nonDuplicates.length > 0) {
                            const retryResult = await usersCollection.insertMany(nonDuplicates, { ordered: false });
                            migrationStats.successfulMigrations += retryResult.insertedCount;
                            console.log(`Inserted ${retryResult.insertedCount} non-duplicate users`);
                        }
                    } else {
...
async function verifyMigration(db) {
    ...
    console.log(`\n${success ? 'SUCCESS' : 'FAILED'} Migration ${success ? 'completed successfully' : 'completed with issues'}`);
}

// Helper function for user input (Node.js version compatible)
...
module.exports = { migrateUsers, migrationStats };